...
"""))
    ...
    def __wallet(self) -> None:
        ...
        player_action = int(input("""Välj från alternativen nedan: 
[1] Insättning (Ej implementerad ännu)
[2] Uttag (Ej implementerad ännu)
[3] Tillbaka
"""))
        if player_action == 1:
            os.system("cls" if os.name == "nt" else "clear")
    ...
    def __deposit(self):
        ...
        print("--- Insättning ---\n")
        deposit_amount = float(input("Ange hur mycket du vill sätta in: "))
        self.player.balance += deposit_amount
        try:
            self.db.update("update_balance", self.player.balance, self.player.username)
            print("insättningen lyckades")
            time.sleep(2)
            os.system("cls" if os.name is "nt" else "clear")
        except:
            print("insättningen misslyckades")
            time.sleep(2)
            os.system("cls" if os.name is "nt" else "clear")
        self.__wallet()
        return
...
"""))   
    ...
    def __change_username(self) -> None:
        ...
        try:
            self.db.update("update_username", new_username, self.player.email)
    ...
    def __change_email(self) -> None:
        ...
        try:
            self.db.update("update_email", new_email, self.player.username)
    ...
    def __change_password(self) -> None:
        ...
        if new_password == confirm_new_password:
            try:
                self.db.update("update_password", new_password, self.player.username)
